...
int rootTest(std::string file = "test.root", double BEAM = 2.2) {
  for (size_t i = 0; i < 6; i++) {
    wq2_s[i] =
        new TH2D(Form("wq2_%zu", i), Form("W vs Q^{2} Sector: %zu", i + 1), 500,
                 0, 3.5, 500, 0, 6.0);
    w_s[i] =
        new TH1D(Form("w_%zu", i), Form("W Sector: %zu", i + 1), 500, 0, 3.5);
  }
  ...
  for (int current_event = 0; current_event < num_of_events; current_event++) {
    if (current_event % 10000 == 0)
      std::cout << "\t" << (current_event * 100 / num_of_events) << "%\r\r"
                << std::flush;
  ...
  std::cout << "Elapsed time for " << num_of_events
            << " events: " << elapsed_full.count() << " s" << std::endl;
  std::cout << "Events/Sec: " << (num_of_events / elapsed_full.count()) << " Hz"
            << std::endl;
...
int main(int argc, char const *argv[]) {
  if (argc < 3) {
    ...
    std::cerr << "To Use:\n\trootTest dst2root_file.root beam_energy"
              << std::endl;
    exit(1);
  }
